...
void Dx12Renderer::InvalidateDeviceObjects()
{
    Dx12_Data *bd = GetBackendData();
    if (!bd || !bd->pd3dDevice)
        return;
...
Dx12Renderer::Dx12_Data *Dx12Renderer::GetBackendData()
{
    return ImGui::GetCurrentContext() ? (Dx12_Data *)ImGui::GetIO().BackendRendererUserData : NULL;
}
...
// Functions
void Dx12Renderer::SetupRenderState(ImDrawData *draw_data, ID3D12GraphicsCommandList *ctx, Dx12_RenderBuffers *fr)
{
    Dx12_Data *bd = GetBackendData();
...
void Dx12Renderer::RenderDrawData(ImDrawData *draw_data)
{
    ...
    Dx12_RenderBuffers *fr = &bd->pFrameResources[bd->frameIndex % bd->numFramesInFlight];
    ...
    SetupRenderState(draw_data, m_pd3dCommandList, fr);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                m_pd3dCommandList->SetGraphicsRootDescriptorTable(1, texture_handle);
                m_pd3dCommandList->RSSetScissorRects(1, &r);
                m_pd3dCommandList->DrawIndexedInstanced(pcmd->ElemCount, 1, pcmd->IdxOffset + global_idx_offset, pcmd->VtxOffset + global_vtx_offset, 0);
            }
        }
...
void Dx12Renderer::CreateFontsTexture()
{
    // Build texture atlas
    ImGuiIO &io = ImGui::GetIO();
    Dx12_Data *bd = GetBackendData();
...
bool Dx12Renderer::CreateDeviceObjects()
{
    Dx12_Data *bd = GetBackendData();
    if (!bd || !bd->pd3dDevice)
        return false;
